...
class BookService:
    ...
    def mostRentedBooks(self): #tested
        ...
        books = self.__repo.sort(books, lambda x, y: x < y)

        books = self.__repo.filter(books, lambda x: x.getRentNumber() > 0)
    ...
    def mostRentedAuthor(self): #tested
        ...
        authorList = self.__repo.sort(authorList, lambda x, y: x[1] < y[1])
        authorList = self.__repo.filter(authorList, lambda x: x[1] > 0)
        return authorList
