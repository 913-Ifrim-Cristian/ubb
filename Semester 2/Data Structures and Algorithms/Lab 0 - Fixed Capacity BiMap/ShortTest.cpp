...
void testAll() { //call each function to see if it is implemented
    ...
    ValuePair keyTest = m.removeKey(432);
    assert(keyTest.first == 69);
    assert(keyTest.second == 25);
    assert(m.search(432) == ans3);

    keyTest = m.removeKey(47);
    assert(keyTest.first == 19);
    assert(keyTest.second == 25);
    assert(m.search(47) == ans3);
