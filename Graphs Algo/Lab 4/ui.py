...
class UI:
    ...
    def _print_menu(self):
        ...
        print("22. Get shortest path between two vertices")
        print("23. Get lowest cost walk between two vertices")
        print("24. Check if DAG, sort topologically the activities and get the maximum cost path")
    ...
    def start(self):
        commands_dict = {
            ...
            "20": self.read_file,
            "21": self.write_file,
            "22": self.get_shortest_path,
            "23": self.get_lowest_cost_walk,
            "24": self.check_if_dga
        }
    ...
    def get_shortest_path(self):
        ...
        end = int(input("Please enter the ending vertex: "))
    ...
    def get_lowest_cost_walk(self):
        start = int(input("Please enter the starting vertex: "))
        end = int(input("Please enter the ending vertex: "))

        dist, prev = lowest_cost_walk(self.graph, start, end)
        path = []
        cost, _end = 0, end
        ...
        print(f"Total cost: {dist[end]}.")

    def check_if_dga(self):
        sorted_list = self.graph.base_topo_sort()
        if sorted_list is None:
            ...
            vertex_begin = int(input("Enter start vertex = "))
            vertex_end = int(input("Enter end vertex = "))
            if vertex_begin not in self.graph.vertices_iterator() or vertex_end not in self.graph.vertices_iterator():
                raise ValueError("ERROR: One vertex/ both vertices entered might not exist.")
            else:
                distance, prev = self.graph.highest_cost_path(vertex_begin, vertex_end)
                if distance == float("-inf"):
                    raise ValueError(f"There is no walk from {vertex_begin} to {vertex_end}")
                path = []
                vertex = vertex_end
                path.append(vertex)
                while prev[vertex] != -1:
                    ...
                    vertex = prev[vertex]
                print(f"The cost of the highest cost path is: {distance}")
                # print the path
