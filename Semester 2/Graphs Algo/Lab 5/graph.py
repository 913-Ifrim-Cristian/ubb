...
class UndirectedGraph:
    ...
    def find_cost(self, x, y):
        ...
        for index in range(0, len(self.costs)):
            if (self.costs[index][0] == x and self.costs[index][1] == y) \
                    or (self.costs[index][0] == y and self.costs[index][1] == x):
    ...
    def dfs_util(self, v, visited, traversal):
        ...
        for neighbour in self._dictionary_bound[v]:
            if neighbour not in visited:
                self.dfs_util(neighbour, visited, traversal)  # go recursively through all the bound vertices
                # that are not visited yet

    def dfs(self, v):
        visited = set()
        traversal = []
        self.dfs_util(v, visited, traversal)
        return traversal
        ...
        if len(line) != 1:
            if int(line[0]) != int(line[1]):  # if the line contains 2 different vertices
                ...
                if int(line[0]) not in graph.dictionary_bound[int(line[1])]\
                        and int(line[1]) not in graph.dictionary_bound[int(line[0])]:
