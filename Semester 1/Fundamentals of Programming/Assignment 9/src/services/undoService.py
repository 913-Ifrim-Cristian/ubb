...
class UndoRedo:
    ...
    def undoRentBook(self, params):
        ...
        bookID = self.__rentalService[rental.ID].book
        clientID = self.__rentalService[rental.ID].client

        self.__bookService[bookID].toggleIsAvailable()
        self.__bookService[bookID].decRentNumber()
        print(self.__clientService[clientID])
        self.__clientService[clientID].toggleActiveRental()
        print(self.__clientService[clientID])
    ...
    def undoUnrentBook(self, rental):
        ...
        bookID = self.__rentalService[rental].book
        clientID = self.__rentalService[rental].client

        self.__bookService[bookID].toggleIsAvailable()
        self.__clientService[clientID].toggleActiveRental()

        self.__clientService[clientID].addDaysRented(-
    ...
    def redoDeleteRent(self, params):
        ...
        if self.__rentalService[item].unrent != date(1, 1, 1):
            self.__clientService[self.__rentalService[item].client].addDaysRented(
                -(self.__rentalService[item].unrent - self.__rentalService[item].rent).days)

        self.__bookService[self.__rentalService[item].book].decRentNumber()
        del self.__rentalService[item]

    def redoUnrentBook(self, rental):

        self.__rentalService.returnBook(self.__rentalService[rental].book, 1)